from coupons.src.repository import CouponRepository, CouponIssueRepository
from coupons.src.entity.coupon_entity import CouponIssueException, ErroCode, CouponEntity
...
class CouPonIssueService:
    ...
    def find_coupon(self, coupon_id: int) -> CouponEntity:
        try:
            ...
            raise CouponIssueException(error_code=ErroCode.COUPON_NOT_EXIST, message="쿠폰이 존재하지 않습니다.")
        return coupon
    ...
    def _check_already_issuance(self, coupon_id: int, user_id: int):
        ...
        if issue is not None:
            raise CouponIssueException(error_code=ErroCode.DUPLICATE_COUPON_ISSUE, message=f"이미 발급된 쿠폰입니다.{coupon_id, user_id}")
